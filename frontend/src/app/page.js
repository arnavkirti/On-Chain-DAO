...
export default function Home() {
  ...
  const daoBalance = useBalance({
    address: CryptoDevsDAOAddress,
    onSuccess: (data) => setDaoBalance(data),
    onError: () => setDaoBalance(null),
  });
  ...
  const numOfProposalsInDAO = useReadContract({
    ...
    onSuccess: (data) => setNumOfProposals(data),
    onError: () => setNumOfProposals(null),
  });
  ...
  async function createProposal() {
    ...
    try {
      ...
      console.log("Transaction result:", tx); // Log the entire transaction object
  
      if (!tx || !tx.hash) {
        throw new Error("Transaction failed or no response received");
      }
  ...
  async function fetchAllProposals() {
    try {
      const numProposals = Number(numOfProposalsInDAO?.data || 0);

      const proposalIds = Array.from({ length: numProposals }, (_, i) => i);
      const proposals = await Promise.all(proposalIds.map((id) => fetchProposalById(id)));
  ...
  async function voteForProposal(proposalId, vote) {
    setLoading(true);
    try {
      // Convert proposalId to a valid format if needed
      const id = BigInt(proposalId);
  
      // Map vote option correctly to smart contract values
      const voteOption = vote === "YAY" ? 0 : 1;
      
      
  
      // Write contract transaction
      const tx = await writeContract(config, {
        address: CryptoDevsDAOAddress, // Ensure this address is correct
        abi: CryptoDevsDAOABI, // Ensure ABI matches the deployed contract
        functionName: "voteOnProposal", // Ensure the function name is correct
        args: [id, voteOption], // Arguments passed to the contract
      });
      ...
      window.alert("Failed to cast vote. Check console for details.");
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className={styles.main}>
        <div>
          ...
          <div className={styles.description}>
            ...
            <br />
            Total Number of Proposals: {numOfProposalsInDAO?.data || 0}
          </div>
          <div className={styles.flex}>
            <button
